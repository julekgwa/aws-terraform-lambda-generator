...
import {Config, LooseObject, Options, Numbers} from '../types/types';
...
function ucFirst(str: string) {
  return str.charAt(Numbers.Zero).toUpperCase() + str.slice(Numbers.One);
}
...
function isUpperCase(myString: string, pos: number) {
  return myString.charAt(pos) === myString.charAt(pos).toUpperCase();
}

function lcFirst(str: string) {
  return str.charAt(Numbers.Zero).toLowerCase() + str.slice(Numbers.One);
}

export function clean(obj: LooseObject) {
  for (const [key, value] of Object.entries(obj)) {
    if (value === '' || value === 'optional' || value === undefined || value === 'false') {
      delete obj[key];
    }
  }
  return obj;
...
export async function createProjectDir(dirPath: string) {
...
export function camelToUnderscore(key: string) {
  ...
  key = isUpperCase(key, Numbers.Zero) ? lcFirst(key) : key;
...
export const getLambdaDirectories = async (source: string) =>
  (await fs.promises.readdir(source, {withFileTypes: true}))
      .filter(
          (dirent) =>
            dirent.isDirectory() &&
...
function createStateType(sfnName: LooseObject) {
  const state: LooseObject = {
    Type: sfnName.stateType
  };

  for (const key in sfnName) {
    if (sfnName.Retry && isJSON(sfnName.Retry)) {
      sfnName.Retry = JSON.parse(sfnName.Retry);
    }

    if (sfnName.Catch && isJSON(sfnName.Catch)) {
...
export function createStateMachineJSON(sfnList: LooseObject[]) {
  if (!sfnList.length) {
    return sfnList;
  }
  const stateMachineJSON: LooseObject = {
...
export async function findScript(scriptPath: string, contents: string) {
  const script = await fs.promises.readFile(scriptPath, 'utf8');

  return script
      ?.trim()
      .includes(contents?.substr(Numbers.Zero, contents?.indexOf('{')).trim());
}

export async function moveTerraScript(
    scriptPath: string,
    find: string,
    replace: string) {
...
async function appendScript(scriptPath: string, contents: string) {
  return fs.promises.appendFile(scriptPath, `\n${contents}`, 'utf8');
}

export async function createTerraformSfn(
    options: Options,
    template: string,
    conf: Config) {
  try {
    const templatePath = `/terraform/${template}.tf`;
    const scriptPath: string = options.currentProjectDir ?
      `${process.cwd()}${templatePath}` :
      `${process.cwd()}/${options.projectName}${templatePath}`;
    let contents: string = conf.terraform[template];

    const stateJson: LooseObject = createStateMachineJSON(options.sfnList);

    contents = contents.replace(
        /:definition/g,
        `definition = jsonencode(${JSON.stringify(stateJson, null, Numbers.Two)
        })`
    );
...
export async function writeTerraformScript(
  ...
  try {
    const templatePath = `/terraform/${template}.tf`;
    const camelCased: string = camelToUnderscore(options.lambda) || '';
    const textToReplace: string = options.new || options.currentProjectDir ?
      '../packages/' :
      '../';
